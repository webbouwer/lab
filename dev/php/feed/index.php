...
{  
		...
		if (isset($token["token_type"]) && $token["token_type"] == "bearer"){
			...
			$arr = array();
		  	...
		  	foreach ($twdata as $t ) {
		   		...
		   		$arr[$i]['description'] = strip_tags( $t->text, '<br>');
		...
		$FBdata = json_decode($FBpage);
			$arr = array();
		  	...
		  	foreach ($FBdata->data as $f ) {
		   		...
		   		$arr[$i]['title'] = strip_tags( $f->story, '<br>');
		   		$arr[$i]['description'] = strip_tags( $f->description, '<br>');
		...
		$WPdata = json_decode($wppostdata);
			$arr = array();
		  	...
		  	foreach ($WPdata as $w ) {
		   		...
		   		$arr[$i]['title'] = strip_tags( $w->title->rendered, '<br>');
		   		$arr[$i]['description'] = strip_tags($w->excerpt->rendered, '<br>');
	...
	*/
	public function socialBundle() {
		...
		$this->fullbundle = array_reverse( $this->socialfeed );
